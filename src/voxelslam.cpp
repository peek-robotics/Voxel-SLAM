                    ...
                    deque<IMU_PRE*>& imu_pre_buf, IMUST& extrin_para, double acc_scale)
    {
        ...
        for (int iterCnt = 0; iterCnt < 10; iterCnt++)
        {
                ...
                iterCnt >= 2)
            {
                ...
                is_degrade = eigvalue[0] < motion_init_eig_threshold_;
...
public:
    ...
    // Wheel odometry sanity check support
    ros::Subscriber sub_wheel_odom_;
    bool wheel_odom_check_enabled_ = false;
    string wheel_odom_topic_ = "/odometry/grover";
    double wheel_odom_ratio_tolerance_ = 0.5;
    double wheel_odom_min_abs_tolerance_ = 0.05;
    ...
    // Local accumulated publishing (rolling buffer)
    struct ScanWithTransform
    {
        pcl::PointCloud<PointType> scan; // undistorted scan in lidar frame
        Eigen::Matrix4f T_lidar_to_map;  // transform from lidar to odom/map
    };
    std::vector<ScanWithTransform> rolling_scan_buffer;
    int rolling_buffer_size_ = 20;
    int current_scan_index_ = 0;
    int total_scans_stored_ = 0;
    // Crop box limits applied per-scan (in lidar frame)
    double cropbox_min_x_ = -50.0, cropbox_max_x_ = 50.0;
    double cropbox_min_y_ = -50.0, cropbox_max_y_ = 50.0;
    double cropbox_min_z_ = -10.0, cropbox_max_z_ = 10.0;
    ...
    string lid_topic, imu_topic, odom_pub_topic;
        ...
        n.param<double>("LocalAccumulated/cropbox_min_y", cropbox_min_y_, -50.0);
        n.param<double>("LocalAccumulated/cropbox_max_y", cropbox_max_y_, 50.0);
        n.param<double>("LocalAccumulated/cropbox_min_z", cropbox_min_z_, -10.0);
        n.param<double>("LocalAccumulated/cropbox_max_z", cropbox_max_z_, 10.0);
        ...
        {
            pub_local_accum_cloud = n.advertise<sensor_msgs::PointCloud2>(local_accumulated_topic, 10);
            rolling_scan_buffer.resize(std::max(1, rolling_buffer_size_));
    ...
    void forceReset(const string& reason, deque<sensor_msgs::Imu::Ptr>& imus,
                    Eigen::Vector3d& last_pos, double& jour, bool& motion_init_flag,
                    bool log_error)
    {
        if (log_error)
    ...
    void AddScanToRollingBuffer(const pcl::PointCloud<PointType>::Ptr& scan, const Eigen::Matrix4f& T_lidar_to_map)
    {
        ...
        {
            if (pt.x >= cropbox_min_x_ && pt.x <= cropbox_max_x_ &&
                pt.y >= cropbox_min_y_ && pt.y <= cropbox_max_y_ &&
                pt.z >= cropbox_min_z_ && pt.z <= cropbox_max_z_)
            {
        ...
        ScanWithTransform swt;
        swt.scan.swap(cropped);
    ...
    void PublishLocalAccumulated(const Eigen::Matrix4f& T_lidar_to_map, double stamp)
    {
        ...
        {
            const auto& swt = rolling_scan_buffer[i];
            Eigen::Matrix4f T_rel = T_current_map_to_lidar * swt.T_lidar_to_map;
            pcl::PointCloud<PointType> transformed;
            pcl::transformPointCloud(swt.scan, transformed, T_rel);
                       ...
                       pcl::PointCloud<PointType>::Ptr pcl_curr)
    {
        ...
        double downkd = down_size >= 0.25 ? down_size : 0.25; // 0.5
    ...
    void thd_odometry_localmapping(ros::NodeHandle& n)
    {
        ...
        while (n.ok())
        {
            ...
            else
            {
                ...
                // Adaptive degeneration counter logic:
                // - Decrement if estimation succeeded AND motion is stable
                // - Increment if estimation failed (regardless of stability)
                // - No change if estimation succeeded but motion is unstable (give system benefit of doubt)
                if (estimation_success && motion_stable)
                {
                    if (degrade_cnt > 0)
                        degrade_cnt--;
                }
...
extern "C" void voxel_slam_start(ros::NodeHandle& n)
{
    ...
    if (enable_loop_closure)
    {
        std::thread thread_loop(&VOXEL_SLAM::thd_loop_closure, vs, std::ref(n));
